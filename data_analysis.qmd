...
appp_data |>
  filter(!is.na(rate_of_pregnant_women_with_oud_diagnosis)) |>
  ggplot(aes(x = rate_of_pregnant_women_with_oud_diagnosis)) +
  geom_histogram(bins = 15, fill = "steelblue", alpha = 0.7, color = "white") +
  geom_vline(aes(xintercept = mean(rate_of_pregnant_women_with_oud_diagnosis, na.rm = TRUE)),
             color = "#e74c3c", linetype = "dashed", size = 1) +
  annotate("text", 
           x = mean(appp_data$rate_of_pregnant_women_with_oud_diagnosis, na.rm = TRUE) - 15,
           y = 10, label = "State Average", color = "#e74c3c", size = 3.5) +
  labs(
    title = "Distribution of OUD Rates Across Pennsylvania Counties",
    subtitle = "Number of counties by OUD rate range",
    x = "OUD Rate per 1,000 Pregnant Women",
